#!/bin/bash
#!
#! Benchmark: GearNet-CA REPA encoder, torch.compile OFF (n=128, bs=80).
#! Runs 200 steps in eager mode and reports per-step wall times for steps
#! 100-200 as steady-state throughput. Pair with bench_ca_repa_compiled.sh.
#!
#! Usage: sbatch hpc-scripts/proteina/smoke_tests/bench_ca_repa_eager.sh

#SBATCH -J bench-ca-eager
...
#SBATCH --time=1:00:00
#SBATCH --qos=gpu1
#SBATCH --output=/rds/user/sr2173/hpc-work/proteina/logs/bench-ca-eager-%j.out
#SBATCH --error=/rds/user/sr2173/hpc-work/proteina/logs/bench-ca-eager-%j.err
#SBATCH -p ampere
...
echo ""
echo "=== Benchmark: GearNet-CA REPA, compile=OFF (200 steps, bs=80) ==="
echo ""
...
def _patched_compose(*a, **kw):
    ...
    try:
        cfg.run_name_ = 'bench_ca_eager_${SLURM_JOB_ID:-0}'
    except Exception:
        pass
    return cfg
...
lines = open('/rds/user/sr2173/hpc-work/proteina/logs/bench-ca-eager-${SLURM_JOB_ID}.out').readlines()
