#!/bin/bash
#!
#! Benchmark: GearNet-CA REPA encoder, torch.compile ON (n=128, bs=80).
#! Runs 200 steps to get past Dynamo warmup (~30-50 steps) and reports
#! per-step wall times for steps 100-200 as steady-state throughput.
#!
#! Usage: sbatch hpc-scripts/proteina/smoke_tests/bench_ca_repa_compiled.sh

#SBATCH -J bench-ca-compiled
...
#SBATCH --time=2:00:00
#SBATCH --qos=gpu1
#SBATCH --output=/rds/user/sr2173/hpc-work/proteina/logs/bench-ca-compiled-%j.out
#SBATCH --error=/rds/user/sr2173/hpc-work/proteina/logs/bench-ca-compiled-%j.err
#SBATCH -p ampere
...
echo ""
echo "=== Benchmark: GearNet-CA REPA, compile=ON (200 steps, bs=80) ==="
echo ""
...
class StepTimer(lpl.Callback):
    WARMUP = 50  # steps to skip before measuring
...
def _patched_compose(*a, **kw):
    ...
    try:
        cfg.run_name_ = 'bench_ca_compiled_${SLURM_JOB_ID:-0}'
    except Exception:
        pass
    return cfg
...
echo ""
# Summarise steady-state step times from STEP_TIME lines
python -u -c "
import sys, re, statistics
lines = open('/rds/user/sr2173/hpc-work/proteina/logs/bench-ca-compiled-${SLURM_JOB_ID}.out').readlines()
times = [float(m.group(1)) for l in lines if (m := re.search(r'elapsed=([0-9.]+)s', l))]
if times:
    print(f'=== STEADY-STATE STEP TIMES (steps 100-200) ===')
