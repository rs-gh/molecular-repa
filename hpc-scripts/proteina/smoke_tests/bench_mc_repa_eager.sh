#!/bin/bash
#!
#! Benchmark: MC-GearNet-Edge REPA encoder, torch.compile OFF (n=128, bs=80).
#! Runs 150 steps in eager mode and reports per-step wall times for steps 50-150.
#! Pair with bench_mc_repa_compiled.sh.
#!
#! Usage: sbatch hpc-scripts/proteina/smoke_tests/bench_mc_repa_eager.sh

#SBATCH -J bench-mc-eager
...
#SBATCH --time=2:00:00
#SBATCH --qos=gpu1
#SBATCH --output=/rds/user/sr2173/hpc-work/proteina/logs/bench-mc-eager-%j.out
#SBATCH --error=/rds/user/sr2173/hpc-work/proteina/logs/bench-mc-eager-%j.err
#SBATCH -p ampere
...
echo ""
echo "=== Benchmark: MC-GearNet-Edge REPA, compile=OFF (150 steps, bs=40) ==="
echo ""
...
def _patched_compose(*a, **kw):
    ...
    try:
        cfg.run_name_ = 'bench_mc_eager_${SLURM_JOB_ID:-0}'
    except Exception:
        pass
    return cfg
...
lines = open('/rds/user/sr2173/hpc-work/proteina/logs/bench-mc-eager-${SLURM_JOB_ID}.out').readlines()
