#!/bin/bash
#!
#! Smoke test: torch.compile on MC-GearNet-Edge REPA encoder (n=128, bs=80).
#! Verifies the encoder compile path works for the more complex MC-GearNet-Edge
#! architecture and measures steady-state step time.
#!
#! Usage: sbatch hpc-scripts/proteina/smoke_tests/smoke_mc_gearnet_encoder_compile.sh

#SBATCH -J smoke-mc-enc-compile
...
#SBATCH --time=2:00:00
#SBATCH --qos=gpu1
#SBATCH --output=/rds/user/sr2173/hpc-work/proteina/logs/smoke-mc-enc-compile-%j.out
#SBATCH --error=/rds/user/sr2173/hpc-work/proteina/logs/smoke-mc-enc-compile-%j.err
#SBATCH -p ampere
...
echo ""
echo "=== Starting MC-GearNet-Edge encoder compile smoke (50 steps, bs=80) ==="
echo ""
...
def _patched_compose(*a, **kw):
    ...
    try:
        cfg.run_name_ = 'smoke_enc_compile_mc_' + str(${SLURM_JOB_ID:-0})
    except Exception:
        pass
    return cfg
