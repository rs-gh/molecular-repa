#!/bin/bash
#!
#! SLURM job script for Proteina training on AFDB Swiss-Prot with 2-GPU DDP.
#! Mirrors hpc-scripts/proteina/training/pdb/train_2gpu.sh but stages the
#! AFDB LMDB from Lustre to local NVMe instead of the PDB LMDB.
#!
#! Effective batch size = num_gpus * datamodule.batch_size * accumulate_grad_batches.
#!
#! Usage:
#!   sbatch .../train_2gpu.sh training_repa_l4_256_afdb_per_residue_bs80_2gpu \
#!                            training/256/afdb/mpnn
...
#SBATCH --output=/rds/user/sr2173/hpc-work/proteina/logs/afdb-2gpu-%j.out
#SBATCH --error=/rds/user/sr2173/hpc-work/proteina/logs/afdb-2gpu-%j.err
#SBATCH -p ampere
...
scontrol update JobId="$SLURM_JOB_ID" JobName="afdb-2gpu-$JOB_SHORT" 2>/dev/null
