"""Build a proper held-out val.lmdb for the Proteina probe suite.

Operating principles (see feedback_cache_intermediates.md):
  1. Commits every --commit_every entries (default 100). If the process dies,
     no more than ~1% of work is lost.
  2. Resumes from partial val.lmdb via the `__ids__` metadata key. Skips
     already-written proteins.
  3. Defensive: exits cleanly on SIGTERM flushing the current buffer;
     never blows away val.lmdb without --force AND explicit confirmation.
  4. Smoke mode: --n_val 10 --output_path /tmp/val_smoke.lmdb exercises the
     full pipeline in ~30 s on the login node.

Usage:
    # CPU login node smoke (writes to /tmp; doesn't touch real val.lmdb):
...
_MAP_SIZE_GB = 10  # val.lmdb with 5K entries sits well under 1 GB; 10 GB gives slack.
...
def main():
    ...
    ap.add_argument(
        "--output_path",
        default=None,
        help="Target LMDB path. Defaults to $data_dir/lmdb/val.lmdb.",
    )
    ...
    ap.add_argument(
        ...
        help="Must match the ratios used to build train.lmdb.",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=100,
        help="Flush batch to LMDB every N parsed entries. Default 100 keeps "
        "work-at-risk under ~2%% even if the process dies mid-build.",
    )
    ap.add_argument(
        "--resume",
        action="store_true",
    ...
    out_path = Path(args.output_path) if args.output_path else (lmdb_dir / "val.lmdb")
    ...
    val_df = dfs_splits["val"].copy()
    val_ids_candidates = set(f"{r.pdb}_{r.chain}" for r in val_df.itertuples())
    logger.info(f"  val candidates: {len(val_ids_candidates)}")
    ...
    val_df = val_df[val_df["pdb"].str.lower().isin(on_disk_pdbs)]
    logger.info(f"  val candidates with raw on disk: {len(val_df)}")

    target_n = min(args.n_val, len(val_df))
    val_df = val_df.sample(n=target_n, random_state=args.seed).reset_index(drop=True)
    logger.info(f"Final requested val set: {len(val_df)} proteins")
    ...
    # _parse_one_structure unconditionally reads graph.bfactor after parsing,
    # so store_bfactor MUST be True. Matches what the hydra-driven
    # convert_to_lmdb.py passes in production.
    # Tuple order: (pdb, chain, protein_id, raw_dir, file_format, store_het,
    #               store_bfactor, apply_coord_reorder)
    worker_args = [
