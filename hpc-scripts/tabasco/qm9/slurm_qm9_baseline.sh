...
if [ -n "$CKPT" ]; then
    ...
    CMD="python hpc-scripts/tabasco/train_tabasco.py experiment=$experiment ckpt_path=$CKPT datamodule.num_workers=0"
else
    echo "Starting fresh training run"
    CMD="python hpc-scripts/tabasco/train_tabasco.py experiment=$experiment datamodule.num_workers=0"
fi
