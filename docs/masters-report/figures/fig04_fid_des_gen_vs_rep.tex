% Figure -- generation-vs-representation envelope (FID + designability). \input{} from Ch6.
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth]{figures/fig04_fid_des_gen_vs_rep.png}
\caption{\textbf{At equal compute, REPA achieves better representation and generation quality.} The dashed arrow marks the matched-compute comparison at step 400K. Against the baseline, the REPA variant has higher fold accuracy (CATH-A) and better generation: lower FPSD (a) and higher designability (b). (Markers are a coarse $\approx$400K-spaced subset of checkpoints from 100K, with the 400K point bordered; $x$ = student CATH-A accuracy, higher to the right.)}
\label{fig:proteina-genrep}
\end{figure}
