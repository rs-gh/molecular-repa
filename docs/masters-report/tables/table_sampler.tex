...
\bottomrule
\end{tabular}
\caption{\textbf{REPA's gains are sampler-invariant in the middle band ($\gamma \in [0.35, 0.5]$) and largely survive at the extremes --- a training-time effect that composes with the inference-time noise dial.} The table gives mean $\Delta$ (REPA $-$ baseline) across step-matched cells at each noise level, for the best PDB and best AFDB variants. Sign reflects effect direction; a negative $\Delta$ is a REPA win on lower-is-better metrics (FID, fJSD-A, ssJSD2D). \colorbox{green!20}{Green} marks an improvement, \colorbox{red!15}{red} a regression. The large $-1225$ FID gain at $\gamma = 0$ reflects the PDB baseline mode-collapsing at deterministic sampling.}
