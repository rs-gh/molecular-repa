% Setup Table S4 -- metric specification across the three measurement families. \input{} from report-draft.tex Ch6.
\begin{table}[tbp]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l >{\raggedright\arraybackslash}p{0.38\textwidth} >{\raggedright\arraybackslash}p{0.20\textwidth} l}
\toprule
\textbf{Metric} & \textbf{Definition} & \textbf{Content} & \textbf{Granularity} \\
\midrule
\multicolumn{4}{l}{\textbf{Representation quality}} \\
IF top-1\,$\uparrow$       & Inverse-folding sequence recovery (top-1 accuracy) & Semantic content & Per residue \\
Dihedral MAE\,$\downarrow$ & Backbone dihedral-angle error & Low-level geometry & Per residue \\
CATH-A top-1\,$\uparrow$   & Fold-architecture classification top-1 accuracy (Table~\ref{tab:setup-cath}) & Fold-level structure & Per chain \\
\midrule
\multicolumn{4}{l}{\textbf{Representation alignment}} \\
CKNNA\,$\uparrow$~\cite{PlatonicRep} & Mutual-nearest-neighbour kernel alignment & Trunk--encoder similarity & Per residue \\
\midrule
\multicolumn{4}{l}{\textbf{Generation quality} (Chapter~\ref{ch:evaluation})} \\
\addlinespace
\multicolumn{4}{l}{\hspace{0.5em}\textit{Quality}} \\
FID\,$\downarrow$          & Fréchet distance to the reference set (GearNet features) & Distributional fidelity & Whole set \\
Des\,$\uparrow$            & Fraction of samples that are designable & Physical realisability & Whole set \\
\addlinespace
\multicolumn{4}{l}{\hspace{0.5em}\textit{Tertiary structure --- whole set (T-W)}} \\
fJSD-A\,$\downarrow$       & Jensen--Shannon divergence vs.\ reference CATH-A distribution & Fold-distribution fidelity & Whole set \\
fS-A\,$\uparrow$           & Spread of generated folds across CATH-architecture classes & Whole-set fold diversity & Whole set \\
pwTM\,$\downarrow$         & Mean pairwise TM-score & Structural diversity & Whole set \\
\addlinespace
\multicolumn{4}{l}{\hspace{0.5em}\textit{Tertiary structure --- designable subset (T-D)}} \\
\#Clust\,$\uparrow$        & Number of TM$\,\ge\,$0.5 structural clusters & Designable diversity & Designable subset \\
pwTM\,$\downarrow$         & Mean pairwise TM-score, designable subset & Designable diversity & Designable subset \\
\addlinespace
\multicolumn{4}{l}{\hspace{0.5em}\textit{Secondary structure --- whole set (S-W)}} \\
ssJSD-2D\,$\downarrow$     & Jensen--Shannon divergence of the (helix-frac, strand-frac) distribution & Secondary-structure fidelity & Whole set \\
\addlinespace
\multicolumn{4}{l}{\hspace{0.5em}\textit{Secondary structure --- designable subset (S-D)}} \\
ssJSD-2D\,$\downarrow$     & As above, on the designable subset & SS fidelity (designable) & Designable subset \\
E\%des\,$\uparrow$         & Mean $\beta$-strand (E) fraction & $\beta$-content & Designable subset \\
\bottomrule
\end{tabular}
\caption{The three measurement families used in this chapter: representation quality,
representation alignment, and generation quality (five reported groups). The whole set is all
generated backbones; the designable subset is those passing the designability filter. Arrows
give the good direction. Full per-metric results appear in
Tables~\ref{tab:proteina-700k} and~\ref{tab:proteina-concentration}.}
\label{tab:setup-metrics}
\end{table}
