...
The two models we study --- Tabasco~\cite{Tabasco} for small molecules and Proteina~\cite{Proteina} for protein backbones (Figure~\ref{fig:domains}) --- are both non-equivariant transformer flow-matching generators, designed to scale with data and model size rather than impose structural priors by construction.
...
A \emph{protein} is a chain of amino-acid residues, ranging from around 50 to several thousands in length~\cite{BritannicaProtein}. Each amino-acid residue contributes four \emph{backbone} atoms (N, C$_\alpha$, C, O) whose interactions determine the chain's \emph{fold} geometry, and a \emph{side chain} that determines its amino-acid identity.
