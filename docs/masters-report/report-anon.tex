...
% Code-availability footnote, gated on the anonymisation flag: the final
% build links the public repository; the blind-marking build points to the
% separately submitted source archive instead (no identifying URL).
\newcommand{\codeavailability}{%
  \ifsubmission
    The source code accompanying this report is submitted as a separate archive alongside this document.%
  \else
    The source code accompanying this report is available at \url{https://github.com/rs-gh/molecular-repa-release}.%
  \fi
}
...
\vspace{42mm}
\textsl{``\@title''}\footnote{\codeavailability}
...
\@title\footnote{\codeavailability}
\vspace{\fill}
...
Main chapters word count: 14942
...
In this chapter, we introduce the machinery the rest of this report depends on: flow matching (\S\ref{sec:flow-matching}), representation alignment (\S\ref{sec:repa}), the de-novo molecular design pipeline (\S\ref{sec:primer}), and the two domains and models we work with (\S\ref{sec:molecular-domains}). The treatment is cursory by design. We intend only to guide the reader through this report, and we provide citations for omitted details.
...
A \emph{protein} is a chain of amino-acid residues, ranging from around 50 to several thousand in length~\cite{BritannicaProtein}. Each amino-acid residue contributes four \emph{backbone} atoms (N, C$_\alpha$, C, O) whose interactions determine the chain's fold geometry, and a \emph{side chain} that determines its amino-acid identity.
...
\begin{itemize}[topsep=2pt,itemsep=1pt]
  \item \emph{context-$\Delta$}: the mean cosine similarity between same-type token pairs minus that between different-type pairs. A positive value means the encoder separates atoms or residues by type. It is the cosine-similarity counterpart to the atom/AA identity probe (\S\ref{sec:profiling-framework}). A high value flags an encoder leaning on token identity, which is cheatable for molecules and potentially off-task for proteins.
