...
\textbf{Designable $\beta$-strand \%.} Designability is gameable. A model can inflate it by emitting easy, highly foldable all-$\alpha$ backbones, since idealised helical bundles re-fold with low scRMSD almost by construction, whereas $\beta$-strand content is harder to render designable. As a guard-rail we therefore track the mean fraction of residues in $\beta$-strand ($f_E$) over the \emph{designable} subset, and read it alongside designability: a designable set that retains $\beta$-rich folds is evidence that its designability is genuine rather than bought with trivial helical artefacts. (We use the C$\alpha$-only secondary-structure assignment of Appendix~\ref{app:ssjsd}.)
...
\textbf{Representation alignment.} Earlier, we outlined how CKNNA scores how closely two representations share their nearest-neighbour structure (\S\ref{sec:eval-probe}). We compute it on clean ($t$=1) data from a fixed held-out batch, between the hidden states at every trunk layer and three domain-relevant encoders: GearNet, ProteinMPNN, and ESM2~\cite{ESM2}. We use $k{=}10$ neighbours over $10{,}000$ residues, and bracket each value with a subsample-without-replacement bootstrap (50 draws at 80\%; sampling with replacement would inflate the score through duplicate neighbour pairs). The absolute CKNNA values we record are small, an order of magnitude below image-domain REPA~\cite{REPA}, so we read the direction and layer structure of the effect, and its separation from the baseline, not its raw magnitude.
...
\textbf{REPA climbs the representation--generation envelope faster than the baseline.} On the representation--generation curve posited by the bottleneck hypothesis, the early representation gains place REPA variants further along than the baseline at matched compute (Figure~\ref{fig:proteina-genrep}, shown for the strongest PDB variant). Hence, it reaches a given level of generation quality at less training. We interpret REPA as simply traversing this curve faster, rather than finding a new shortcut.

\input{figures/fig04_fid_des_gen_vs_rep}

\textbf{This representation--generation link is encoder-matched, suggesting the effect is encoder-specific, not generic regularisation.} Each encoder's generation gain lands on the axis where its representations are strongest. GearNet, the better fold encoder (\S\ref{sec:proteina-rep}), gives the larger acceleration in fold distribution; MPNN, the better local encoder, gives the larger acceleration in designability (Table~\ref{tab:proteina-speedup}). Such axis-matching would be unlikely under a generic auxiliary loss, but we do not claim proof of causality.
...
\subsection{In the long run, the gains are dataset- and encoder-dependent}
\label{sec:proteina-distribution}
\label{sec:proteina-anatomy}
\label{sec:proteina-modulation}

The short-run acceleration is universal: every variant, in both regimes (\S\ref{sec:proteina-convergence}). The long run is not. Pushed deep into training, what REPA does --- and how much --- splits along two axes. The first is the \emph{dataset} the trunk is trained on, which sets how much room the baseline leaves for improvement. The second is the \emph{encoder} it is aligned to, which selects \emph{which} representational factor improves. We therefore stop reading the long-run metrics pooled across regimes, and take each in isolation. This is where the experimental--synthetic distinction that motivates our two-regime design (Chapter~\ref{ch:intro}, \S\ref{sec:proteina-datasets}) becomes the lens through which the long run is read. Two exhibits carry it: the long-run column of Table~\ref{tab:proteina-speedup} records each variant's best quality (designability and FPSD), and the 1.3M snapshot below (Table~\ref{tab:proteina-13m}) lays out the full metric breakdown behind those numbers.

\input{tables/table_proteina_13m}

\textbf{Because the dataset fixes the baseline's headroom, it fixes how large and lasting REPA's advantage is.} Figure~\ref{fig:proteina-fid} plots the strongest variant in each regime against the baseline and the random control. Where the baseline plateaus poorly, the lead is large and lasting, as on AFDB FPSD. Where the baseline is a slow learner, the lead narrows yet still holds, as on PDB designability. Only where the baseline is a strong learner, as on PDB FPSD, does the lead fade: the baseline catches up and eventually closes the gap. AFDB designability leaves little headroom over the baseline. As noted earlier (\S\ref{sec:proteina-datasets}), this is likely a quirk of how designability is measured and how AFDB was generated, not any special merit. Tellingly, even the random control scores highly here.

\textbf{On synthetic data, REPA learns a genuinely better model.} Read on its own, AFDB is the clean regime. At the 1.3M snapshot, the strongest AFDB variant, REPA-L4-GearNet, beats the baseline on designability, distribution match (fJSD-A), fold spread (fS-A) and secondary structure (ssJSD-2D) at once --- nearly the whole row moves in REPA's favour (Table~\ref{tab:proteina-13m}). This is a true lift to the learned distribution, not a sampler artefact: it survives the deterministic ODE limit, which reads the model with no injected noise (Table~\ref{tab:proteina-ode}). And it is durable: the distribution-match lead still holds at the end of training, REPA-L4-GearNet's best FPSD $282$ against the baseline's $386$ (Table~\ref{tab:proteina-speedup}, long-run column). It also comes without a diversity cost --- the designable subset is no more concentrated than the baseline's. Even the random-weights control improves the whole-set distribution here, but only the learned encoders turn that into better per-sample quality. One caveat: the AFDB designability lift is partly proxy-flavoured (\S\ref{sec:proteina-datasets}), so the FPSD lift is the cleaner part.

\input{tables/table_ode}

\textbf{On experimental data, REPA accelerates and redistributes, rather than learning a uniformly better model.} PDB is the mixed regime. The whole-set diversity gain holds --- the generated set still spreads more widely than the baseline's deep into training (fS-A) --- but the FPSD lead is transient: the baseline closes it by ${\sim}1.4$M (\S\ref{sec:proteina-convergence}), and its best FPSD ends up just ahead of every REPA variant's (Table~\ref{tab:proteina-speedup}, long-run column). The designability lead, by contrast, is durable --- REPA-L9-MPNN's best designability, $0.82$, stays well above the baseline's $0.67$ (same column). What changes most is the designable subset: REPA pushes it toward $\beta$-rich folds and concentrates it onto fewer of them. At 400K almost every PDB variant concentrates the most $\beta$-strand-rich designable bin ($\beta{\ge}25$) above the baseline, and the concentration is still clear at the 1.3M centerpoint, where REPA-L9-MPNN sits at pairwise TM ${\approx}0.69$ in that bin against the baseline's ${\approx}0.13$ (Table~\ref{tab:proteina-concentration}). This is REPA-wide, not one encoder, and it is onto $\beta$-rich rather than easy helical folds, so the designability is not bought with trivial all-$\alpha$ backbones. Whether the concentration persists or relaxes very late is unsettled on our current seeds, so we read the late designable bins as directional, not precise. We track diversity by pairwise TM, not the raw cluster count of Table~\ref{tab:proteina-13m}, for the sample-size reason given in \S\ref{sec:eval-protein}.

\input{tables/table_concentration}

\input{figures/fig5_diversity_trend}

\textbf{The second axis is the encoder, and which encoder wins is itself dataset-dependent.} Each encoder boosts a different factor: GearNet lifts the fold distribution while ProteinMPNN lifts per-sample designability (\S\ref{sec:proteina-convergence}). Which one helps most then follows the dataset's bottleneck. PDB's baseline is weakest on designability, so ProteinMPNN wins there --- REPA-L9-MPNN is the strongest PDB variant. AFDB's baseline plateaus on distribution match, so GearNet wins --- REPA-L4-GearNet is the strongest on AFDB (Table~\ref{tab:proteina-13m}). That choosing an encoder is really choosing which factor to enhance --- the reading that makes REPA a family of interventions --- we develop in the discussion (\S\ref{sec:proteina-discussion}).
...
\textbf{Does alignment outlive its purpose?} The same encoder pull that broadens the whole set may be what keeps the designable set concentrated. Relative to baseline, REPA broadens the folds the model generates over the whole set, but may concentrate the designable subset onto fewer modes. This concentration appears early in the baseline as well as in REPA. The two then part ways: the baseline grows out of the early regime and diversifies, while REPA stays concentrated, so the cost relative to baseline widens deep into training rather than appearing there for the first time.

One interpretation may be that the encoder is a fixed target, so once the trunk has extracted the representational signal it had to offer, continued alignment keeps pulling toward the encoder's own distribution, which for our structure encoders is comparatively narrow. In this framing, the gain and the cost are not separate phenomena, but early and late phases of one pull.

Indeed, a similar arc is reported in image-diffusion literature, and a proposed solution is early-stopping alignment~\cite{HASTE}. We do not test such a schedule and make no argument of this form, but on this reading, an early stop may let REPA variants diversify like the baseline while keeping the whole-set gains.

\textbf{REPA is a family of encoder-mediated interventions, not a single mechanism.} Our results suggest that in a domain with multi-factorial representation, choosing an alignment target implicitly chooses what factor gets boosted. There is no single best encoder; each buys a different representational factor, so the optimal outcome is likely use-case specific. The effect is doubly conditional (\S\ref{sec:proteina-modulation}): the dataset sets how much room there is to improve, but it is the encoder that decides \emph{which} factor improves --- the axis that makes this a family of interventions rather than a single lever.
