...
Main chapters word count: 14904
...
Table~\ref{tab:setup-datasets} summarises the two datasets used in Chapter~\ref{ch:proteina-study}. Both training sets are $\alpha/\beta$-dominated and span a substantial fraction of CATH architectures (44.2\% PDB, 61.8\% AFDB), which is what makes the per-chain CATH fold probe meaningful. Neither random split is sequence-clustered: 98.3\% of PDB and 92.2\% of AFDB validation chains have a $\ge$30\%-identity homolog in their own training set, so absolute in-house scores are inflated and we read rank-order and $\Delta$-from-baseline instead. The cross-database overlap is far lower: only 62.1\% of AFDB validation chains have a $\ge$30\%-identity hit in PDB training. This is what makes a clean evaluation possible. Filtering AFDB chains clean against \emph{both} training pools leaves $\sim$325 proteins at $n{\le}256$, more than five times the $\sim$62 left in the same-source PDB validation set, so this cross-database set serves as the blinded probe set for PDB-trained models (\S\ref{app:leakage}; Chapter~\ref{ch:evaluation}).

\section{Choice of CATH fold-probe level}
\label{app:cath}

We report CATH \emph{architecture} (CATH-A) as the headline fold-probe level throughout Chapter~\ref{ch:proteina-study}. Table~\ref{tab:setup-cath} justifies that choice: Class is too coarse (four buckets, one already near-saturated), Topology too finely-bucketed to probe reliably, and Architecture sits between --- discriminative but learnable. The example here shows the PDB-trained baseline trunk ($n{\le}256$): the probe classifies validation chains by their CATH code and we report the linear-probe top-1 accuracy at each level.
...
However, we note this might reflect a measurement limit. These probes are run on the small cross-database blinded set at a single seed and a single checkpoint, so step-to-step noise is large. For example, the baseline dihedral error alone swings several degrees between adjacent checkpoints. We therefore draw no per-residue conclusion on AFDB and lean on the fold result, where the effect is large enough to clear that noise.
...
Proteina's secondary-structure metric compares the \emph{mean} helix and strand content of a generated set against a reference. This collapses each set to a single point, so it cannot see variance, bimodality, or tails. Hence, a model that matches the average composition while collapsing its spread scores the same as one that reproduces it. ssJSD-2D removes this blind spot by comparing the full joint distribution.
...
Tables~\ref{tab:profiling-mol} and~\ref{tab:profiling-protein} in the main text show only the diagnostics that drive each verdict. Tables~\ref{tab:profiling-full-mol} and~\ref{tab:profiling-full-protein} give the complete measured set for every candidate encoder. The probe, \emph{3D}, \emph{Floor}, \emph{Lift}, \emph{Sparsity} and \emph{RankMe} columns carry the same meaning as in the main-text tables. The added columns are \emph{context-$\Delta$} (the within-minus-between cosine spread); \emph{Best} (the best-projector cosine, with $\text{Lift}=\text{Best}-\text{Floor}$); \emph{PR} (participation ratio); \emph{Dims@95\%} (dimensions holding $95\%$ of the variance); and \emph{norm} (mean L2 embedding norm). The spectral columns --- RankMe, PR, Dims@95\% and norm --- diagnose optimisation tractability. Rank collapse or a norm explosion degrades the gradient signal even when the embedding is informative.
