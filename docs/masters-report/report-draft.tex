...
Main chapters word count: 14997
...
However, for both our datasets, validation sets are heavily homologous to their own training data. For example, 98\% of PDB validation chains have a $\ge$30\% hit in PDB training, leaving only $\sim$62 clean chains at $n{\le}256$, which is far too few to probe. Hence, we look across datasets to increase the count. AFDB chains filtered clean against \emph{both} the PDB and AFDB training pools yield $\sim$325 proteins, more than $5\times$ the same-source PDB slice.

This number sufficees for our per-residue probes, as the $\sim$325 proteins still yield $\sim$43k residues, which yields full statistical power. However, the per-chain CATH probe is different. Fold classification needs many distinct chains spread across populated fold classes, and the blinded slice is too small and too class-sparse for this. Hence, for the numbers we report in the main text, we relax our clean condition to the probe-clean split on the fuller validation set ($\sim$3{,}190 chains). Comparing the splits also decomposes the leakage --- the dirty-to-probe-clean drop isolates probe leakage and the probe-clean-to-blinded drop isolates model leakage --- and the REPA-over-baseline gap survives in every regime.
...
Table~\ref{tab:proteina-rep-afdb} is the AFDB-trained counterpart of the PDB Table~\ref{tab:proteina-rep}. The fold routing (GearNet\,$\to$\,CATH) replicates. The per-residue routing does not: inverse folding and the dihedral probe show no consistent REPA effect, unlike the clear MPNN-led per-residue gains on PDB. We read this as a measurement limit, not a real absence. These probes run on the small cross-database blinded set at a single seed and a single checkpoint, so step-to-step noise is large --- the baseline dihedral error alone swings several degrees between adjacent checkpoints. We therefore draw no per-residue conclusion on AFDB and lean on the fold result, whose effect is large enough to clear that noise.
...
The encoder-routing reading in \S\ref{sec:proteina-anatomy} --- GearNet shifting the designable subset toward $\beta$-strand content, MPNN on AFDB away from it --- rests on the secondary-structure composition of the designable backbones (Table~\ref{tab:ss-composition}). The shifts are small in absolute terms but seed-consistent: on AFDB the L9-GearNet and L9-MPNN strand and helix ranges do not overlap across seeds, in opposite directions. The strand column ($f_E$) is the same designable $\beta$-strand metric as the centerpiece $\beta$-strand \% column (Table~\ref{tab:proteina-13m}), shown here per-seed and alongside the helix axis. We report it at 700K rather than the centerpiece's 1.3M because that is the step at which the load-bearing AFDB L9-GearNet vs.\ L9-MPNN contrast has multi-seed coverage on both encoders --- the AFDB L9-GearNet run trains only to 1.0M, so it has no 1.3M checkpoint. The per-seed values therefore do not match the 1.3M centerpiece cell exactly; we report the spread here rather than in the main table to avoid clutter.
...
The deterministic (ODE) sampler reads the learned velocity field with no injected noise, so it isolates what the trunk has learned from the sampler that draws from it. Table~\ref{tab:proteina-ode} gives the all-variant ODE snapshot at 1.3M. On AFDB the lift holds across every trained encoder, not just the strongest one, and without concentrating the designable subset (des.\ pwTM); this is the ODE evidence behind the ``genuinely better model'' reading of \S\ref{sec:proteina-anatomy}. On PDB the floor instead redistributes, raising designability while the distribution-match gain is mixed.
