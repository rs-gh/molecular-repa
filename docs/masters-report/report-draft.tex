...
Main chapters word count: 14731
...
\textbf{Generation quality.} This is what ultimately matters, and we report it through the two-axis taxonomy of Chapter~\ref{ch:evaluation} --- tertiary/secondary structure crossed with whole set/designable subset --- laid out as supercolumns in our result tables: per-sample quality, then T-W, T-D, S-W, and S-D (Table~\ref{tab:eval-protein}). We report two headline metrics: designability and FPSD. We use this taxonomy to analyse REPA's effect on both the whole generated distribution and the designable subset, along with the trade-off the latter implies.
...
%                            the whole-set distribution-match/diversity beat AND the ODE-
%                            floor result (keystone). The designability-distribution-match
