...
Main chapters word count: 14767
...
\chapter*{Abstract}
The evolution of generative machine learning for de-novo molecular design has increasingly favoured prior-light models, trading hard structural symmetries for architectural simplicity and synthetic data scale. While this enables broader scaling, it dilutes the structural signal available to the model during training. This exacerbates the \emph{representation bottleneck}, where the objective must simultaneously learn how to represent complex geometry and generate samples. In practice, this dual burden leads to slow convergence and expensive training budgets.

Our report investigates Representation Alignment (REPA), a technique from the imaging domain that injects soft inductive biases by aligning a generator's hidden states with the embeddings of a frozen, pretrained encoder. We systematically characterise when and why it succeeds in the molecular domain.
...
\emph{Reading figures and tables in this chapter.} Unless otherwise specified, figures plot data from the PDB-trained $n\le 256$ data-regime (\S\ref{sec:proteina-datasets}); trajectories show the seed mean, with shaded bands giving the min/max range. Each float notes its seed count in a trailing parenthetical. In tables, the baseline is usually an absolute value and each REPA row a $\Delta$ from it; \colorbox{green!20}{green} marks a change in the metric's good direction and \colorbox{red!15}{red} a change against it; \colorbox{green!42}{darker green} signals the best entry in a column; small $|\Delta|$ are left uncoloured.
...
\textbf{Each data regime leaves a different gap, and REPA fills the one it is given.} On synthetic (AFDB) data, designability comes readily, partly by construction (\S\ref{sec:proteina-datasets}), while matching the fold distribution is the harder problem. On experimental (PDB) data the reverse holds, as the baseline already matches the distribution well, but leaves room on designability. REPA's long-run gain then shows up wherever the gap is: on distribution match for AFDB, on designability for PDB (Figure~\ref{fig:proteina-fid}).

\input{tables/table_proteina_13m}

\textbf{On synthetic data, REPA learns a genuinely better model.} At the 1.3M snapshot, the strongest long-run AFDB variant (Table~\ref{tab:proteina-speedup}, long-run column), REPA-L4-GearNet, beats the baseline on Quality and whole-model (T-W, S-W) metrics at once, \emph{at no meaningful cost to designable-subset metrics} (T-D and S-D) (Table~\ref{tab:proteina-13m}). This represents a true pushing of the designability--diversity envelope (\S\ref{sec:protein-tradeoffs}). The $0.02$ rise in designable pwTM is within the baseline's own seed-to-seed scatter (Appendix~\ref{app:13m-ranges}), while the designability quality gains clear it. This makes REPA-L4-GearNet the canonically better model under that framework, not merely a different point on the same envelope. We note that this is learned model behaviour, not a sampler artefact. The same pattern survives the deterministic ODE limit, which reads the velocity field with no injected noise (Appendix~\ref{app:ode}).

\textbf{On experimental data, REPA finds a different spot on the designability--diversity envelope.} At the 1.3M snapshot, across variants, REPA delivers whole-model gains (T-W, S-W) while pushing the designable subset toward $\beta$-strand content (S-D) (Table~\ref{tab:proteina-13m}). However, designable diversity (T-D) drops, as the model appears to collapse onto fewer distinct $\beta$-rich folds (Table~\ref{tab:proteina-concentration}). This $\beta$-driven dynamic of secondary-structure enhancement and loss of tertiary-structure diversity is far stronger on PDB. The performance of the strongest long-run variant, REPA-L9-MPNN (Table~\ref{tab:proteina-speedup}, long-run column), makes the resultant trade-off clear: it trades diversity for designability. On PDB, REPA finds another spot on this envelope, instead of pushing the frontier out as with AFDB.

\input{tables/table_concentration}

\textbf{The optimal alignment target follows from the data regime.} Alignment with GearNet lifts the fold distribution, while alignment with ProteinMPNN lifts per-sample designability (\S\ref{sec:proteina-convergence}). PDB's gap is in designability, so ProteinMPNN wins there, with REPA-L9-MPNN proving the best long-run PDB variant. AFDB's gap is in distribution match, so REPA-L4-GearNet proves stronger. For REPA in a multi-factorial and multi-scale domain like molecules, the data regime dictates which encoder pays off.
...
\textbf{One thread we leave open is whether alignment outlives its early purpose.} REPA helps most early, and on PDB the distribution-match lead is transient while the designable subset concentrates mid-training (\S\ref{sec:proteina-anatomy}). One reading is that the encoder is a fixed target, so once the trunk has taken what it offers, continued alignment keeps pulling it toward the encoder's own, narrower distribution. A similar early-helps arc is reported for image diffusion, where alignment is best stopped early~\cite{HASTE}. We see a superficially similar pattern, but our evidence on whether the late cost is permanent is too thin to say the same remedy applies here.
