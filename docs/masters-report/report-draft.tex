...
\subsection{In the short run, REPA accelerates generation quality}
...
\subsection{In the long run, REPA consistently dominates baseline on whole-set, but not designable-subset metrics}
...
% BEAT B1 (consistency + persistence of the whole-set gain). Centerpiece shown
% at 1.3M (post the ~1.0M designability dip). Numbers from the clean pipeline:
% PDB MPNN-L9 @1.3M dDes +0.22 / dFPSD -29 / dfS-A +0.84; AFDB GN-L4 @1.3M
% dFPSD -71 / dfS-A +2.14. Four non-headline cells pending (job 30352629).
\textbf{REPA consistently improves whole-set distribution match and diversity, and the gain is durable.} Under almost every REPA variant tested, the generated set matches the reference fold distribution more closely (fJSD-A) and spreads more widely across fold classes (fS-A); secondary-structure distribution match (ssJSD-2D) improves likewise. Curiously, this holds for the random-weights control too, but only the learned encoders translate it into better per-sample quality (Table~\ref{tab:proteina-13m}). The advantage is not a transient head start: the whole-set lead persists deep into training, and at 1.3M the strongest PDB variant (REPA-L9-MPNN) again leads designability by $+0.22$.

\textbf{However, REPA's designable subset stays more concentrated than the baseline's.} Despite the whole-set gains, REPA's designable-subset pwTM rises above the baseline as training proceeds, even as its whole-set diversity exceeds the baseline's (Figure~\ref{fig:proteina-diversity-trend}).
...
One interpretation may be that the encoder is a fixed target, so once the trunk has extracted the representational signal it had to offer, continued alignment keeps pulling toward the encoder's own distribution, which for our structure encoders is comparatively narrow. In this framing, the gain and the cost are not separate phenomena, but early and late phases of one pull. Indeed, the image-diffusion literature reports a similar arc, and early-stops alignment for it~\cite{HASTE}. Nevertheless, in this report, we do not investigate the mechanism behind the data we observe, and we do not claim early-stopping would help.

\textbf{REPA's gains interact with the training dataset --- durable on synthetic data, transient on experimental data.} The intervention is identical across datasets, yet the asymmetry tracks the baseline's headroom. On experimental (PDB) data the FPSD acceleration is real but transient, and the baseline eventually catches up. On synthetic (AFDB) data, where the baseline plateaus poorly, the whole-set distribution-match lead is large and durable --- and it survives even the deterministic ODE limit, which reads the learned distribution with no injected noise. At 700K the AFDB ODE floor lifts FPSD from $220$ to ${\sim}147$ and designability from $0.12$ to ${\sim}0.34$, whereas the PDB floor lifts at 700K but regresses by 1.0M (Table~\ref{tab:proteina-ode}). The representation gap that opens early and never closes (\S\ref{sec:proteina-rep}) is consistent with REPA genuinely improving the distribution the AFDB-trained trunk learns. One caveat: the AFDB designability lift is partly an artefact of the proxy's AlphaFold lineage (\S\ref{sec:proteina-datasets}); the FPSD lift is the clean, proxy-independent part.
