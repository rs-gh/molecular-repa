...
Main chapters word count: 14845
...
We now turn to the designability--diversity trade-off (\S\ref{sec:protein-tradeoffs}). The picture here has two parts, which the current literature framing tends to merge. Along the classic trade-off itself --- designability against designable-subset diversity --- REPA does not push the envelope outward; it \emph{moves along} it, raising designability while conceding diversity within the designable subset. Separately, and on a different axis, REPA improves the whole model distribution: it is more diverse and closer to the reference. We read these observations as qualifiers that add context to the main convergence result above (\S\ref{sec:proteina-convergence}).
...
\textbf{However, a REPA-learned distribution may collapse onto fewer folds within its designable subset.} This is REPA moving along the designability--diversity trade-off of \S\ref{sec:protein-tradeoffs}: it raises designability, but its designable subset is less diverse. Across most configurations, that subset is structurally more concentrated than the baseline's, as measured by the mean pairwise TM-score (pwTM, the designable-subset diversity metric of \S\ref{sec:eval-protein}).
...
Here, we turn to a second relationship between our headline metrics. This one is not a characteristic trade-off of the domain, but an empirical frontier we observe in the sampler sweep: designability against whole-set fidelity. We find that REPA configurations are strictly better in this regard than baseline.

\textbf{REPA navigates the same designability--fidelity frontier as the baseline.} As outlined in \S\ref{sec:flow-matching}, Proteina samples with an SDE whose noise scale $\gamma$ trades designability for whole-set fidelity (FPSD). This frontier is distinct from the designability--diversity trade-off whose anatomy we dissected in \S\ref{sec:proteina-anatomy}. It is an inherent property of the sampler, and REPA does not reshape it. As $\gamma$ rises, both REPA and the baseline shed designability and improve whole-set fidelity together (Table~\ref{tab:proteina-sampler}).
