...
Main chapters word count: 14688
...
In this subsection, we outlined our headline results using the \emph{Quality} metrics of our suite, FPSD and designability (as in Table~\ref{tab:eval-protein}). A deeper analysis of generation quality along tertiary- and secondary-structure decomposition follows in \S\ref{sec:proteina-anatomy}.
...
\subsection{REPA's gains are robust to sampler noise}
\label{sec:proteina-sampler}

\textbf{The FPSD and designability gains we observe hold across a sampler noise ablation.} At almost every $\gamma$ setting, REPA reaches a higher level of \emph{both} FPSD and designability than the baseline (Table~\ref{tab:proteina-sampler}). The exception is the degenerate $\gamma{=}0$ regime, where the baseline saturates designability by repeating a few folds. This suggests that REPA's gains are built into the learned model, not merely recovered at inference.

\input{tables/table_sampler}

\textbf{REPA raises the deterministic-sampling floor, the strongest sign of a better velocity field.} The probability-flow ODE samples the trunk's learned distribution without injected noise (\S\ref{sec:flow-matching}), the closest view of the true learned distribution before any correction. Under the baseline, that distribution is barely designable: Des $0.04$ on PDB and $0.12$ on AFDB. But REPA's learned distribution is already far more designable at ODE, across every learned encoder (Table~\ref{tab:proteina-ode}). This suggests REPA mechanistically improves the distribution the trunk learns.
...
\subsection{REPA consistently dominates baseline on whole-set, but not on designable-subset metrics}
\label{sec:proteina-distribution}
\label{sec:proteina-anatomy}

Two facts sit side by side here, and they are not in tension. Over the whole generated set, REPA genuinely improves the distribution: more diverse, and closer to the reference. But within the \emph{designable} subset, it may only be moving along the designability--diversity envelope (\S\ref{sec:protein-tradeoffs}), trading some of that subset's diversity for the designability it gains. We take the whole-set win first, then the designable-subset cost.

\input{tables/table_proteina_700k}

% BEAT B1 (consistency + persistence of the whole-set gain). Sampler-robustness
% moved to its own subsection right after the headline-acceleration section
% (6.2.3). Numbers verified against the clean pipeline 2026-06-08: PDB MPNN-L9
% @1.0M dFPSD -81 / dfS-A +0.29; AFDB GN-L4 @1.0M dFPSD -228 / dfS-A +3.68.
\textbf{REPA consistently improves whole-set fidelity and diversity, and the gain is durable.} Under almost every REPA variant tested, the generated set matches the reference fold distribution more closely (fJSD-A) and spreads more widely across fold classes (fS-A); secondary-structure fidelity (ssJSD-2D) improves likewise. Curiously, this holds for the random-weights control too, but only the learned encoders translate it into higher FPSD and designability (Table~\ref{tab:proteina-700k}). The advantage is not a transient head start. The whole-set lead persists deep into training, with the strongest variant on each dataset still ahead at 1.0M steps --- REPA-MPNN-L9 on PDB (FPSD $-$81, fold-score $+0.29$) and REPA-GN-L4 on AFDB ($-$228, $+3.68$) --- even as its designable-subset diversity stays below baseline.

\textbf{However, REPA's designable subset displays fold concentration.} Despite the whole-set gains, within the designable subset, REPA's pwTM rises above the baseline at almost every checkpoint, and sharply so by 1.0M steps (Table~\ref{tab:designable-diversity}).
