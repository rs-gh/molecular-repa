...
Main chapters word count: 14757
...
% ==========================================================================
% RESTRUCTURE SCAFFOLD (2026-06-08). Split the old muddled subsection into TWO
% cleanly-themed ones:
%   Section A (this one)  -- the designability-diversity ENVELOPE: REPA moves
%                            ALONG it, does not push it out. Evidence: designable
%                            pwTM (n-independent; #Clust is confounded by
%                            sample count -- see exp_wholeset_vs_designable_
%                            diversity.py) shows REPA's designable subset is MORE
%                            concentrated than baseline at almost every step.
%   Section B (below)     -- REPA improves the WHOLE-MODEL DISTRIBUTION. Absorbs
%                            the whole-set fidelity/diversity beat AND the ODE-
%                            floor result (keystone). The designability-fidelity
%                            "frontier" framing is DROPPED -- implicit in "we
%                            learn a better model"; the sampler table is kept as
%                            supporting evidence under B.
% VERIFIED METRIC NOTE: use designable pwTM, NOT #Clust, for the diversity
% claim (pwTM is n-independent). pwTM data lives in
% results/variance/wholeset_vs_designable_diversity.json. Earlier "MPNN-L9
% beats both late" was a #Clust sample-count artifact; pwTM shows REPA
% concentrated at almost every variant/step. So the honest claim is the simple
% one: REPA moves along the envelope, does not push it.
% ==========================================================================

\subsection{REPA moves along the designability--diversity envelope}
\label{sec:proteina-anatomy}

We now turn to the designability--diversity trade-off (\S\ref{sec:protein-tradeoffs}). Relative to baseline, REPA does not push this envelope outward. It finds a new spot on the same envelope, raising designability while conceding diversity within the designable subset, much as the sampler dial does. We read this as a qualifier to the main convergence result (\S\ref{sec:proteina-convergence}), not a contradiction of it.

\textbf{REPA's whole set stays diverse, but its designable subset concentrates.} The distinction matters, because diversity is conventionally read off the designable subset alone (\S\ref{sec:eval-protein}). We measure structural diversity by mean pairwise TM-score (pwTM); lower is more diverse, and because it is an average it compares directly across models and sample counts. Over the whole generated set, REPA's pwTM tracks the baseline's --- it is no less diverse, consistent with its higher fold-spread (fS-A). But within the \emph{designable} subset, REPA's pwTM rises above the baseline at almost every checkpoint, and sharply so by 1.0M steps (Table~\ref{tab:designable-diversity}). The concentration is therefore specific to the designable modes, not the whole distribution. The lone near-exception is last-layer ProteinMPNN at 700K, whose designable subset is marginally more diverse than the baseline's; we do not read a single point as a trend.

\input{tables/table_designable_diversity}

% BEAT A2 -- encoder routing / beta-concentration (KEEP, lightly). The
% concentration is severe for beta-rich samples; AFDB-MPNN escapes via helix
% routing. Ties to the encoder-routing theme.
\textbf{The concentration is encoder-dependent and severe for $\beta$-rich folds.} In our data, the concentration appears especially severe for $\beta$-strand-rich samples (Table~\ref{tab:proteina-concentration}). The REPA construction appears to generate the \emph{same} $\beta$-rich folds repeatedly, while the baseline spans many. Notably, the AFDB-MPNN variant does not display this: its designable subset stays as diverse as the baseline's in every $\beta$-bin (Table~\ref{tab:proteina-concentration}). One explanation is that encoder routing leads to different secondary- and tertiary-structure distributions; this MPNN configuration nudges the learned distribution towards helices, which may be why it escapes this largely strand-heavy phenomenon (Appendix~\ref{app:ss-composition}).

\input{tables/table_concentration}

% BEAT A3 -- PDB early/late (KEEP). Baseline concentrates early then grows out;
% REPA stays concentrated. Single-seed directional at late steps.
\textbf{On PDB, even the baseline concentrates early in training, but grows out of it.} After $700$K training steps, both the PDB baseline and its REPA variants concentrate their $\beta$-rich designable samples, though REPA already does so more. By 1.0M steps, the baseline has diversified while REPA stays concentrated, widening the gap (Table~\ref{tab:proteina-concentration}). This suggests REPA does not \emph{engender} the concentration so much as fail to grow out of an early-training regime the baseline leaves behind. We read the late checkpoints as a single-seed directional pattern, not a precise effect size.

% BEAT A4 -- early-stopping question (KEEP, natural close to the concentration
% story). Note the early/late arc and the HASTE parallel; we do not test it.
\textbf{Does alignment outlive its purpose?} Given REPA's early acceleration in generation quality and the later-stage fold concentration we observe, the best approach may be to stop alignment after early training. A similar arc is reported in the image-diffusion literature, where REPA helps early and is best early-stopped~\cite{HASTE}. We do not test such a schedule and make no argument of this form, but on this reading an early stop may let REPA diversify like the baseline while keeping the whole-set gains.

\subsection{REPA improves the whole-model distribution}
\label{sec:proteina-sampler}

Separately from where it sits on the designability--diversity envelope, REPA improves the whole learned distribution. Per generated set, the proteins it produces are more diverse over the whole set, and their distribution sits closer to the reference.

\input{tables/table_proteina_700k}

% BEAT B1 (merged: consistency + persistence + sampler-robustness -- three facets
% of one "the gain is real and stable" claim). Numbers verified against the clean
% pipeline 2026-06-08: PDB MPNN-L9 @1.0M dFPSD -81 / dfS-A +0.29; AFDB GN-L4 @1.0M
% dFPSD -228 / dfS-A +3.68. The old "designability-fidelity frontier" framing is
% dropped; the sampler table is supporting evidence here, not a separate frontier.
\textbf{REPA consistently improves whole-set fidelity and diversity, and the gain is durable.} Under almost every REPA variant tested, the generated set matches the reference fold distribution more closely (fJSD-A) and spreads more widely across fold classes (fS-A); secondary-structure fidelity (ssJSD-2D) improves likewise. Curiously, this holds for the random-weights control too, but only the learned encoders translate it into higher FPSD and designability (Table~\ref{tab:proteina-700k}). The advantage is not a transient head start: the whole-set lead persists deep into training, with the strongest variant on each dataset still ahead at 1.0M steps --- REPA-MPNN-L9 on PDB (FPSD $-$81, fold-score $+0.29$) and REPA-GN-L4 on AFDB ($-$228, $+3.68$) --- even as its designable-subset diversity stays below baseline. It is also robust to the sampler. As the noise scale $\gamma$ rises, REPA and the baseline shed designability and improve FPSD together. Yet at every $\gamma$ except the degenerate $\gamma{=}0$ regime, REPA reaches a higher level of \emph{both} than the baseline (Table~\ref{tab:proteina-sampler}).

\input{tables/table_sampler}

% BEAT B2 -- ODE floor = better learned distribution (KEYSTONE, kept SEPARATE).
% ODE samples the learned distribution with no noise correction. Baseline barely
% designable at ODE; REPA far more designable at ODE across learned encoders.
% => REPA mechanistically improves the learned distribution. THE PUNCHLINE.
\textbf{REPA raises the deterministic-sampling floor, the strongest sign of a better learned distribution.} The probability-flow ODE samples the trunk's learned distribution without injected noise (\S\ref{sec:flow-matching}), the closest view of the true learned distribution before any correction. Under the baseline, that distribution is barely designable: Des $0.04$ on PDB and $0.12$ on AFDB. SDE sampling rescues this (baseline designability climbs to ${\approx}0.84$ on PDB at $\gamma{=}0$). But REPA's learned distribution is already far more designable at ODE, across nearly every learned encoder (Table~\ref{tab:proteina-ode}); only the PDB random control fails to lift it ($-0.02$). This suggests REPA mechanistically improves the distribution the trunk learns.
