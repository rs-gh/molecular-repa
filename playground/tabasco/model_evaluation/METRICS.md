...
All metrics used to evaluate the quality of generated molecules from our flow matching models, across training validation and the standalone evaluation pipeline.

## Summary Table

Two collection contexts:
- **Training val** — computed automatically during training every 1000 global steps, on 100 generated samples. This is what runs in the current pipeline.
- **Post-hoc eval** — computed once per checkpoint by `evaluate.py` on 1000 generated samples with bootstrap CIs. This is what we are doing now to get reliable comparisons.

| # | Metric | Training Val (n=100) | Post-hoc Eval (n=1000) | Observed Spread | Saturation |
|---|--------|---------------------|----------------------|----------------|------------|
...
- **When:** Training validation: every 1000 global steps, on 100 generated samples. evaluate.py: once on 1000 samples with bootstrap CI.
...
- **When:** Training validation: NOT computed. evaluate.py only: n=1000 with bootstrap CI.
...
- **When:** Training validation: NOT computed. evaluate.py: once, no bootstrap CI.
...
- **When:** Training validation only (every 1000 steps, n=100). NOT in evaluate.py.
...
2. **Noise exceeds signal at n=100:** For every metric with non-zero spread between models, the epoch-to-epoch noise (+/-5-20%) exceeds the inter-model spread (0-7%). No statistically reliable conclusions possible from training validation.
