...
¹ Re-run 2026-06-04 (after fixing a venv drift — rogue `rdkit-pypi 2022.9.5` shadowed the locked `rdkit 2025.9.3`, so the LMDBs would not depickle). Now RankMe = `exp(H(p))` with `p = σᵢ / Σσ`, the same metric as MACE's 40.6, so the two are directly comparable. **Higher is not better here**: CheMeleon's rank is high because its variance is *diffuse and sparse* (93.8% zeros, 500 dims for 90% variance), not collapsed — the opposite failure mode to MC-GearNet's rank collapse. The headline conditioning failure is the 93.8% sparsity itself. The old stale value was `138 = exp(H(p))` with `p = σᵢ² / Σσ²` (the old proteina convention); discard it.
...
|---------------|--------------|--------------------|
| **CheMeleon** | **unusable as a 3D-REPA target** | **Q1.2 catastrophic**: identical embeddings for all conformers (L2 = 0.000) — REPA cannot teach 3D geometry. **Q3.1 catastrophic**: 93.8% sparsity (ReLU) — cosine operates over ~130 active dims of 2048, gradients dominated by activation pattern. **Q3.2**: variance is diffuse (500 dims for 90%), not collapsed — a different conditioning issue from rank collapse, but the sparsity (Q3.1) is the headline conditioning failure. (NB: earlier drafts framed this as "overflows the 128-d projector input"; that figure was the bare hidden_dim of one small config, ignored the 2x coord+atom concat, and is not used — dropped.) **Q1.1 strong** (probe 1.000) but trivial in QM9 (max 9 atoms). **Q2 floor 0.43 with gap +0.04** — the gap exists, but it's 2D bond-graph context, conformation-invariant, useless for 3D guidance. |
