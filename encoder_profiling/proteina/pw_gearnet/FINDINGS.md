...
**Date**: 2026-04-29 (torsional variant). Structure-denoising last profiled 2026-04-24 (job 28308561) and is **not** included in the latest standardised sweep — see "Structure variant" section below.
...
- **Torsional variant — borderline.** Eff rank 12.2 / 3072. Projector saturation gap = +0.009 (mean-dir 0.710 → best 0.719). 3D-sensitive (cos = 0.92 at 0.5 Å). Strong AA identity signal (probe 0.927). Usable if no better option, but ESM2 (gap +0.053) and CA-GearNet (3D headroom in geometric terms) are both stronger choices.
- **Structure variant — borderline-poor (stale numbers).** Last run 2026-04-24: eff rank 19.9, mean-dir 0.781, onehot 0.794 → gap ~0.013. Numbers below were collected before the audit (`projector_num_layers` 2→3 fix and per-residue averaging change), so they aren't directly comparable to the new pipeline.
...
## Torsional variant — full numbers (2026-04-29)

### 1. Value distribution & dimensionality
...
| Residue-type shuffle | 0.995 ± 0.001 |

3D-sensitive (0.5 Å → cos 0.92, vs MC's 0.81 and CA-GearNet's 0.37). Fully rotation-invariant. Residue-shuffle cos 0.995 means the embedding is dominated by structure, not residue identity — even though residue type is in the node feature.
...
Probe is high (0.927) because residue type is a node-feature input, but the centroid-cos 0.977 and Δ ~0.01 say the per-AA cluster centers are nearly indistinguishable in direction — identity is encoded by axis-aligned subspace differences too small to dominate cosine similarity.

### 4. Structural context (SS) sensitivity
...
**Saturation gap = +0.009.** Above the random-init floor (+0.003) but well below ESM2 (+0.053). The picture is: identity + position takes the projector to 0.72; coordinates can add another ~0.01.
...
### 8. Layer-wise representation

From [layerwise.csv](results/torsional_20260429_140213/layerwise.csv):
...
## Structure variant (stale — 2026-04-24)

⚠ The numbers below are from job 28308561 (2026-04-24), **before** the proteina REPA audit (`projector_num_layers` 2 → 3, per-residue averaging fix). They are NOT directly comparable to the 2026-04-29 torsional numbers above. To bring this variant into the standardised comparison, run:
...
Old top-line metrics:

| Metric | Structure (stale) |
|--------|------------------:|
| Effective rank | 19.9 |
| Mean L2 norm | 22.7 |
| Dead dims | 22 / 3072 |
...
| AA probe accuracy | 0.96 |
| Mean-dir baseline | 0.781 |
| Best projector (onehot) | 0.794 |
| Estimated headroom | ~0.013 |

Qualitatively: structure-denoising has a higher mean-dir baseline (0.78 vs torsional's 0.71) and weaker SS-specific delta — its pretraining task compressed the hidden space harder than torsional did. Verdict: torsional remains preferred.
...
1. **Prefer CA-GearNet** as the REPA target for 3D content. PW has higher absolute cosine similarity but most of it is mean-direction baseline; CA-GearNet's lower baseline leaves more room for the *geometric* signal we actually want REPA to teach.
2. If we explicitly want a 3D + sequence-aware encoder, **PW torsional** is the usable variant. Its saturation gap (+0.009) is comparable to CA-GearNet's (+0.006) and the torsional pretraining task gives it the cleanest SS signal of any GearNet variant we've checked.
