"""Per-pair envelope: SS 2D-JSD (designable) vs designability rate — n=128.

Mirrors plot_ssjsd_vs_des_envelope_per_pair.py but reads from the
n128_convergence sweeps and uses the n128 run-name conventions. See that
script for the axis-orientation / arrow / bubble-size / pretrained-overlay
rationale — kept identical here for visual consistency across sample sizes.

Two figures produced — one per SS-JSD reference set:
...
FIG_OUT = REPO_ROOT / "evaluation/proteina/generation/figures/paper/n128_convergence"
FIG_OUT.mkdir(parents=True, exist_ok=True)

# Pre-committed step at which to draw the baseline → REPA arrow. Matches
# plot_ssjsd_vs_des_envelope_per_pair (n256) and plot_gen_vs_rep_envelope_per_pair.
TARGET_STEP = 400_000
...
def plot_one(ref: str) -> None:
    ...
    for row, (ds_name, ds_cfg) in enumerate(DATASETS.items()):
        ...
        base_pts = [(k[1], *gen_map[k]) for k in base_keys]

        # AFDB has 3 variants but the grid is 4-wide → blank the extra cell.
        for col in range(n_cols):
            if col >= len(ds_cfg["repa_variants"]):
                axes[row, col].axis("off")
                continue
            rep_prefix, rep_label, rep_color, rep_marker = ds_cfg["repa_variants"][col]
            ax = axes[row, col]
            ...
            if SHOW_PRETRAINED:
                pre = pretrained_overlay.load_gen("n128")
    ...
    suptitle = (
        f"n=128 — SS 2D-JSD vs designability envelope per baseline–REPA pair\n"
