...
def build_cleantrain(max_size: int, out_dir: Path):
    print(f"\n[cleantrain max_size={max_size}] {out_dir}")
    ...
    write_manifest(
        out_dir / "batch_manifest_train_clean_v2.json",
        version="train_clean_v2",
...
def build_xclean(lmdb_dir: str, max_size: int, out_dir: Path, label: str):
    print(f"\n[xclean {label} max_size={max_size}] {out_dir}")
    ...
    write_manifest(
        out_dir / "batch_manifest_train_v2.json",
        version="train_v2",
...
def main():
    ...
    build_cleantrain(128, RESULTS / "n128_convergence_cleantrain_pdb")
    build_cleantrain(256, RESULTS / "n256_convergence_cleantrain_pdb")

    # xclean: uniform sample of train.lmdb (no filter)
    # n128/n256 xclean_afdb_pdb: PDB-trained model -> AFDB train pool -> AFDB val clean
    build_xclean(AFDB_LMDB, 128, RESULTS / "n128_xclean_afdb_pdb", label="afdb_train")
    build_xclean(AFDB_LMDB, 256, RESULTS / "n256_xclean_afdb_pdb", label="afdb_train")
    # n256 xclean_pdb_afdb: AFDB-trained model -> PDB train pool -> PDB val clean
    build_xclean(PDB_LMDB, 256, RESULTS / "n256_xclean_pdb_afdb", label="pdb_train")
