...
def load_rep(path: Path) -> pd.DataFrame:
    ...
    cath: Dict[Tuple[str, int], float] = defaultdict(lambda: -np.inf)
    ...
    with path.open() as fh:
        for r in csv.DictReader(fh):
            ...
            if kind == "cath" and r.get("cath_level") == "T":
                v = fnum(r.get("cath_accuracy"))
                if v is not None and v > cath[key]:
                    cath[key] = v
        ...
        rows.append(
            {
                "run": run,
                "step": step,
                "cath_T_top1": cath.get((run, step))
                if np.isfinite(cath.get((run, step), -np.inf))
                else None,
