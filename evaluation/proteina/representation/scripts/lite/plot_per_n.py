...
Usage:
  python evaluation/proteina/representation/scripts/lite/plot_per_n.py
"""
...
# scripts/lite/plot_per_n.py -> parents[1] = representation/
REPR_ROOT = HERE.parents[1]
RESULTS = REPR_ROOT / "results" / "lite"
FIG_DIR = REPR_ROOT / "figures" / "lite"

SIZES = ["n128_lite", "n256_lite", "n512_lite"]
TS = [0.50, 0.75, 1.00]
...
def _samples_seen(size_dir: str, family: str, step: int) -> int | None:
    ...
    if size_dir == "n256_lite":
        return step * 12
    if size_dir == "n512_lite":
        if family == "baseline":
            return step * 6
...
SIZE_LABEL = {"n128_lite": "n = 128", "n256_lite": "n = 256", "n512_lite": "n = 512"}
...
def _plot_grid(metric: str, ylabel: str, out_path: Path) -> None:
    ...
    (FIG_DIR / "contact").mkdir(parents=True, exist_ok=True)
    (FIG_DIR / "cath").mkdir(parents=True, exist_ok=True)
...
def main():
    _plot_grid(
        "p_at_L_5_linear",
        "P@L/5 (linear)",
        FIG_DIR / "contact" / "fig_grid_P_at_L5.png",
    )
    _plot_grid("cath_acc", "CATH accuracy", FIG_DIR / "cath" / "fig_grid_cath_acc.png")
