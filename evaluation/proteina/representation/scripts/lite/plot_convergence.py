"""Convergence-sweep plots (Pipeline A, n=512 multi-checkpoint).

Reads the multi-step sweep CSV at
`results/convergence/n512_convergence/sweep_results.csv` and emits three
figures into `figures/convergence/n512/`:
...
Usage:
  python evaluation/proteina/representation/scripts/convergence/plot_convergence.py
"""
...
# scripts/convergence/plot_convergence.py -> parents[2] = representation/
REPR_ROOT = HERE.parents[1]
CSV = REPR_ROOT / "results" / "lite" / "n512_convergence_lite" / "sweep_results.csv"
FIG_DIR = REPR_ROOT / "figures" / "lite" / "n512_convergence_lite"
...
def _plot_progression(
    df: pd.DataFrame, metric: str, ylabel: str, title: str, out_path: Path
) -> None:
    """Single-metric: probe `metric` vs samples processed, one curve per run at its aligned layer.
    ...
    enc_vals = _encoder_values(df, metric)
    for run, val in enc_vals.items():
        # Skip NaN reference lines - breaks log-scale tight_layout and gives a
        # meaningless "(frozen)" entry in the legend. Happens commonly on CATH
        # when the 40-protein subset has too few labels.
    ...
    ax.set_xlabel("Samples processed (step x batch_size)")
    ax.set_xscale("log")
...
def main():
    ...
    _plot_layerwise(
        ...
        FIG_DIR / "contact" / "fig_layerwise_P_at_L5.png",
    )
    _plot_layerwise(
        df,
        "cath_acc",
        "CATH fold accuracy - layer-wise (final checkpoint per run)",
        FIG_DIR / "cath" / "fig_layerwise_cath_acc.png",
    )
    _plot_progression(
        df,
        "p_at_L_5",
        "P@L/5",
        "Contact P@L/5 vs training progress",
        FIG_DIR / "contact" / "fig_step_progression.png",
    )
    _plot_progression(
        df,
        "cath_acc",
        "CATH accuracy",
        "CATH acc vs training progress",
        FIG_DIR / "cath" / "fig_step_progression.png",
    )
