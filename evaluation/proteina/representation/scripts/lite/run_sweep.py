...
# scripts/lite/run_sweep.py -> parents[1] = .../representation
# (parent of `lib/`; `from lib import ...` resolves here)
...
def main():
    ...
    ap.add_argument("--output_dir", type=str, default=str(HERE.parents[1] / "results"))
    ap.add_argument(
        "--consolidate_only",
        action="store_true",
    ...
    if pre.config is not None:
        ...
        cfg_path = HERE.parents[1] / "sweep_config.yaml"
    ...
    # representation/ directory (HERE.parents[1]), not the CWD, so they work regardless
    # of where the job is launched from.
    outdir = _outdir if _outdir.is_absolute() else HERE.parents[1] / _outdir
