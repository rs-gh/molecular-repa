...
def _draw_run(ax, pts, color, marker, label):
    ...
    # Tiny step label next to each bubble. Offset in points so it stays
    # legible at any zoom; gray + small font keeps clutter manageable.
    for step, x, y in pts:
        ax.annotate(
            f"{step // 1000}k",
            xy=(x, y),
...
def plot_metric(metric_key: str, rep_key: str) -> None:
    ...
    for row, (ds_name, ds_cfg) in enumerate(DATASETS.items()):
        ...
        if lower_better:
            # Invert once per shared-y row (acts on all axes in the row) so
            # "up and to the right = better" holds across all plots.
